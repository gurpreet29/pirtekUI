...
;$(document).ready(function() {
	...
	//for faqs plus minus icons
	$('.collapse').on('shown.bs.collapse', function(){
		$(this).parent().find(".close_accord").removeClass("close_accord").addClass("open_accord");
	}).on('hidden.bs.collapse', function(){
		$(this).parent().find(".open_accord").removeClass("open_accord").addClass("close_accord");
	});
	...
	// faq tab active
    $('.leftside-menu-mobile ul li a').click(function(){
        $('a[href="' + $(this).attr('href') + '"]').tab('show');
        $('.leftside-menu-mobile ul li').removeClass('active');
   ...
   // active class in navigation-B
    var pgurl = window.location.href.substr(window.location.href.lastIndexOf("/")+1);
     $(".navbar-inverse ul li a").each(function(){
	...
	$(".mobile-panel li a").click(function(){
        var activeValue = $(this).html();          
        $(".leftside-menu-mobile button#dLabel").html(activeValue + '<span class="caret"></span>');
        $(".leftside-menu-mobile button#topdropdown").html(activeValue + '<span class="caret"></span>');
    });

	//news video show more button
	$('.news-video .collapsed').click(function() {
		$(this).hide();
	});

	$('.thank-u-modal .close').click ( function() {
		$('.disclaimers .panel-title span').removeClass('glyphicon glyphicon-minus-sign').addClass('glyphicon glyphicon-plus-sign');
		$('.disclaimers .panel-collapse').removeClass('in');
	});
    ...
    //for left panel
    $(".leftside-menu ul li.active").prev("li").css("border","none");
...
var app = {
	init: function() {
		...
		this.defineCarInsuranceBoxes();
		this.defineOffWhiteBoxes();
		this.defineDownloadBoxes();
		this.defineVideoHeight();
		this.swipeCarousel();
        this.defineFormheight();
		app.affixNav.init();
	...
	initPageElements: function() {
        ...
        $('.featured-container .video-icon').click(this.playFeaturedVideo);
        $('.closeModalVideo').click(this.closeFeaturedVideo);
    },
    ...
    defineSearchPanel: function() {
    	...
    	if( windowWidth < 950 ) {
			$('#global-search .search').css( { 'min-width': windowWidth } );
		} else {
			$('#global-search .search').css( { 'min-width': '950px' } );
		}
    },
    ...
    defineVideoHeight: function() {
		if( $('video').length ) {
			$('video').each(function() {
				var height = $(this).parents('.carousel').height();
		...
		if( $('iframe').length ) {
			$('iframe').each(function() {
				...
				if( $(this).parents('#toyota-secondary-banner').length || $(this).parents('.inner-hub-pages').length  ) {
					$(this).css({'max-height': height});
					$(this).parents('.videoContainer').css({'max-height': height});
				} else {
					if( $(window).width() > 767 ) {
						$(this).css({'max-height': height - 50});
						$(this).parents('.videoContainer').css({'max-height': height - 50 });
					} else {
						$(this).css({'max-height': height});
						$(this).parents('.videoContainer').css({'max-height': height });
					}
    ...
    //featured videos
    playFeaturedVideo: function(e) {
    	var modalId = $(this).parent('a').attr('data-target');
    		modal = $(modalId);
    ...
    embedNewsVideo: function() {
        ...
        if( videoSrc !== undefined ) {
            video = '<video id="video" width="100%" controls autoplay>'+
                ...
                video = '<iframe width="100%" height="350" src="'+iframeSrc+'?enablejsapi=1&version=3&playerapiid=ytplayer&rel0&autoplay=1&wmode=transparent" frameborder="0" allowfullscreen></iframe>';
        }
    ...
    defineCarInsuranceBoxes: function() {
    	$(".white-shadow-box").css({'min-height': 0});

    	var max = -1;
    	$(".white-shadow-box").each(function() {
		    var h = $(this).height(); 
		    max = h > max ? h : max;
		});
		$(".white-shadow-box").css({'min-height': max+15});
    },

    defineOffWhiteBoxes: function() {
    	$(".offwhite-shadow-box").css({'min-height': 0});

    	var max = -1;
    	$(".offwhite-shadow-box").each(function() {
		    var h = $(this).height(); 
		    max = h > max ? h : max;
		});
		$(".offwhite-shadow-box").css({'min-height': max+15});
    },
    ...
    defineFormheight: function(){
        if($(window).width() > 767){
            $(".display-section-fieldset").css({ 'min-height': 0 });
            var max = -1;
            $(".display-section-fieldset").each(function () {
                var h = $(this).outerHeight();
                max = h > max ? h : max;
            });
            $(".display-section-fieldset").css({ 'min-height': max });           
        }
    }
};
...
$(window).load(function() {
	...
	app.defineCarInsuranceBoxes();
    app.defineSearchPanel();
	app.defineOffWhiteBoxes();
	app.defineDownloadBoxes();
    app.defineFormheight();
	if ( $('.featured-container').length ) {
...
;app.affixNav = {
	init: function() {
		...
		this.globalNav = $('header');
		this.topNav = $('#topNav');
		this.leftNav = $('.leftside-menu-mobile .dropdown');
		this.affixNav = $('#affixnav');

		//defining affix element
		if( this.leftNav.length && $('.faqs ,.contact').length <= 0 ) {
			this.navElement = this.leftNav;
		} else if( this.topNav.length ) {
			this.navElement = this.topNav;
		} else {
			this.navElement = this.globalNav;
		}
	},
	...
	initNavElements: function() {
		...
		if( $(window).width() < 768 ) {
			if(( this.scroll >= this.navElement.offset().top + this.navElement.height() ) && ( this.scrollState === 'top' )) {

				//showing defined element
				if( this.leftNav.length && $('.faqs ,.contact').length <= 0 )  {
					this.leftNavigation();
				} else if( this.topNav.length ) {
					this.subNavigation();
			...
			} else if(( this.scroll < this.navElement.offset().top + this.navElement.height() ) && ( this.scrollState === 'scrolled' )) {
				this.hideAffixNav();
			}
		}
	},
	...
	navigation: function() {
		var globalNav = this.globalNav.clone();
		globalNav.find('button.navbar-toggle').attr("data-target", "#globaldropdown");
		globalNav.find('.navbar-collapse').attr("id", "globaldropdown");

		this.affixNavHTML = globalNav;
		this.showAffixNAv();
	},

	subNavigation: function() {
		var topNav = this.topNav.clone();
		topNav.find('button').attr("data-target", "#Newnav");
		topNav.find('div#finance-navbar-collapse').attr("id", "Newnav");
		this.affixNavHTML = topNav;
		this.showAffixNAv();
	},

	leftNavigation: function(){
		var leftnav = this.leftNav.clone();
		leftnav.find('button').attr("id", "topdropdown");
		leftnav.find('ul.mobile-panel').attr("aria-labelledby", "topdropdown");	

		this.affixNavHTML = leftnav;
		this.showAffixNAv();
	},

	hideAffixNav: function() {
		this.affixNav.hide().animate({top:'-55px'}, 100);
        this.scrollState = 'top';
	},

	showAffixNAv: function() {
		if( this.affixNav.html() == '' ){
			this.affixNav.html(this.affixNavHTML);
		}
		this.affixNav.show().animate({ top:'0px' }, 100);
